    ...
    public void loadData(Scanner scanner) {
	    ...
	    */
	    private void solve(int row, int col) throws IOException {
	        ...
	        else {  // Loop through the character array and check.
	        	for (int i = 0; i < characters.length; i++) {
	                if (appearInRow(row, characters[i])) { continue; }  // character already appears in the same row.
	                if (appearInCol(col, characters[i])) { continue; }  // character already appears in the same column.
	                if (appearInGrid(row, col, characters[i])) { continue; }  // character already appears in the 3-by-3 grid.
	                board[row][col] = characters[i];  // Fill the cell with the character.
